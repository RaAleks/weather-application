...
public class GeoDecoderService {
    ...
    public LocationResponse decodeCity(String city, String apiKey) {
        String url = String.format("/geo/1.0/direct?q=%s&limit=1&appid=%s", city, apiKey);

        return weatherRestClient.get().uri(url).retrieve().body(LocationResponse.class);
    }
    ...
    public WeatherResponse fetchWeather(double latitude,
        ...
        String url = String.format("/data/2.5/weather?lat=%s&lon=%s&appid={API key}", latitude, longitude, apiKey);
